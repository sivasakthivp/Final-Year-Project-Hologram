...
class Volumetric3DCNNStatistical:
    ...
    def extract_features(self, volume: np.ndarray) -> np.ndarray:
        """
        Extract features from a 2D or 3D volume.
        Inputs:
        - 2D image: (H, W) → Expands to (1, H, W)
        - 3D volume: (D, H, W) → Uses as-is
        ...
        try:
            ...
            if len(volume.shape) == 2:
                logger.info(f"Expanding 2D image {volume.shape} to 3D")
    ...
    def _morphological_features(self, volume: np.ndarray) -> list:
        ...
        try:
            ...
            # Threshold to create binary mask (process slice-by-slice for 3D)
            vol_normalized = ((volume / (volume.max() + 1e-8)) * 255).astype(np.uint8)
            if len(vol_normalized.shape) == 3:
                binary = np.zeros_like(vol_normalized)
                for i in range(vol_normalized.shape[0]):
                    _, binary[i] = cv2.threshold(vol_normalized[i], 100, 255, cv2.THRESH_BINARY)
            else:
                _, binary = cv2.threshold(vol_normalized, 100, 255, cv2.THRESH_BINARY)
