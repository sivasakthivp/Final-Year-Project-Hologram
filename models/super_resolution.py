...
class GANSuperResolutionOpenCV:
    ...
    def enhance(self, image: np.ndarray) -> np.ndarray:
        """
        Enhance a 2D or 3D image with 4x upscaling.
        Inputs:
        - 2D image: (H, W) → Returns (H*4, W*4)
        - 3D image: (D, H, W) → Returns (D, H*4, W*4)
        """
        try:
            ...
            if len(image.shape) == 2:
                ...
                upscaled = self._upscale_slice(image)
                logger.info(f"Super-resolution complete. New shape: {upscaled.shape}")
                return upscaled
